...
|---|---|
| Version | v0.11.24 |
| Status | **Docs-sync session (2026-06-13)** — catch-up cho 4 versions đã commit vào master nhưng chưa từng có LAST_SESSION/PROJECT_PROGRESS entry: **FID-VN-020 (ORCH-001/CT-011)** ✅ DONE (v0.11.20) — `scripts/orchestrator.py::detect_confusion/create_consultation_request/close_session` + 11 tests. **CT-054** ✅ regenerate `data/eval/bench_002b_results.json` baseline (phát hiện+fix GT spacing bug → Drug Recall 55.6%→61.5%, Precision→100%, Diag CEER 71.4%→21.4%, Vitals CEER→28.9%), **CT-055/CT-056** ✅ áp dụng baseline mới + đóng pending liên quan. Thêm **FID-VN-021** (CT-060d, L1b phoneme-key re-scoring, `fids/FID-VN-021.md` Andy approved) implemented trên `experiment/fid-vn-021-phoneme`: `src/core/vn_phoneme.py` (decompose_syllable/phoneme_key) + `_fuzzy_match()` rewrite (`src/core/l1b_drug_correct.py`, phoneme-key 2nd pass, `PHONEME_WEIGHT=0.9`). A/B benchmark byte-identical vs baseline → quyết định CT-028 precedent: **research note, KHÔNG merge** vào master. |
| Tests | **984/984 PASS** (master) — 1009/1009 trên `experiment/fid-vn-021-phoneme` · bandit src/ 0 HIGH (9 MEDIUM/2 LOW pre-existing, không đổi) |
...
| Next task | **TRAIN-001** full fine-tune (9207 samples VietMed, nhiều epoch) khi có GPU/quota tốt hơn (Kaggle 30h/tuần) hoặc pilot audio · CT-049 re-test · CT-019 debug A2-VAD nếu có audio · các PA UI test khác |
