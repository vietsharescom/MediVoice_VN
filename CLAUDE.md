...
|---|---|
| Version | v0.11.26 |
| Status | **TRAIN-001 tooling session (2026-06-13)** — chuẩn bị eval + notebook để chạy full fine-tune khi có GPU/pilot audio. Thêm `scripts/eval_asr_phowhisper.py` (WER eval, base hoặc fine-tuned checkpoint, output JSON `data/eval/train001_eval_results.json`) + `notebooks/TRAIN_001_colab_kaggle.ipynb` (notebook gộp đủ 10 bước: clone→install→VietMed→pilot audio optional→manifest→smoke-test→full fine-tune→eval WER baseline vs fine-tuned→download checkpoint+cleanup) + cập nhật `docs/dev/COLAB_KAGGLE_TRAINING.md` (step 9b). **CT-061** ✅ thêm pilot audio retention opt-in (`src/core/l0_normalize.py::retain_pilot_audio`, flag `pilot_audio_retention` trong `config/facility_config.json`, default `false`) — khi `true`, `/api/transcribe` copy audio+transcript vào `data/audio/pilot/` trước purge, cho phép BS ghi âm thật tại phòng khám (local FastAPI, PhoWhisper+PhoBERT an toàn) tích lũy data cho TRAIN-001. Phiên trước (v0.11.24): docs-sync catch-up FID-VN-020/CT-054/055/056 + FID-VN-021 (research note). |
| Tests | **998/998 PASS** (991 + 7 mới cho pilot audio retention) · bandit src/ 0 HIGH (9 MEDIUM/2 LOW pre-existing, không đổi) |
| Pending | **CT-053** Vietnamese Medical Phonetic Encoder (Phase 2, chờ pilot audio) · **CT-027** Ciprofloxacin "si pô lo siêu âm si" alias (cần thêm audio mẫu) · **CT-049** (Andy re-test clip TMH lần 3) · **PA-020/PA-021** (Andy test UI FID-VN-017/018) · **PA-015/PA-017/PA-018** (Andy test FID-VN-013/014/015/016 UI) · **CT-019** (🔴 A2 VAD-chunk regression) · CT-016/CT-017/CT-014/CT-035/CT-036/CT-037/CT-039/CT-044 ⏳ · **bật `pilot_audio_retention: true`** tại facility pilot khi BS bắt đầu ghi âm thật (Andy xác nhận consent) |
| Next task | **TRAIN-001** bật `pilot_audio_retention: true` tại `config/facility_config.json`, chạy `install.bat`/`start.bat` tại phòng khám, BS ghi âm thật qua `localhost:8000` → tích lũy `data/audio/pilot/` → khi đủ, chạy `notebooks/TRAIN_001_colab_kaggle.ipynb` trên Colab/Kaggle · CT-049 re-test · CT-019 debug A2-VAD nếu có audio · các PA UI test khác |
| Design | `docs/records/DESIGN_REPORT_v1.1_20260606.md` (§15 v2.1) · FID: `fids/FID-VN-007.md`, `fids/FID-VN-010.md`, `fids/FID-VN-012.md`, `fids/FID-VN-013.md`, `fids/FID-VN-014.md`, `fids/FID-VN-015.md`, `fids/FID-VN-016.md`, `fids/FID-VN-017.md`, `fids/FID-VN-018.md`, `fids/FID-VN-019.md`, `fids/FID-VN-020.md`, `fids/FID-VN-021.md` (research note) |
