...
|---|---|
| Version | v0.11.19 |
| Status | **CT-042 DONE ✅ (2026-06-12)** — FID-VN-019 v3 (`fids/FID-VN-019.md`, Andy approved via CONS-20260612-001 Groq+ChatGPT+Grok) implemented: `_phonological_variants()` (`src/core/l1b_drug_correct.py`) — 4 rule groups (aspiration b↔p/d↔t/g↔k/c↔k/đ→t, coda drop {l,z,v,d,đ} multi-syllable, "th"→t/d, r/l/n region-split) + guards (first+last syllable, ambiguity-guard, min-length, 43-word blacklist). `_build_alias_map()` 1028→1913 keys (+885), 21 collisions skipped. +15 tests (973/973 PASS), bandit 0 HIGH. A/B benchmark (`tools/bench_002b.py`, experiment branch): Drug Recall 0.556 unchanged, Drug Precision 0.714 unchanged vs master HEAD (verified pre-existing, unrelated — see CT-054). Merged fast-forward to `master` (commit `13aff3a`). **FID-VN-020 (ORCH-001/CT-011) APPROVED by Andy (2026-06-12)** — Orchestrator automation (`detect_confusion`/`create_consultation_request`/`close_session`), implementation chưa bắt đầu (next session). |
| Tests | **973/973 PASS** · bandit src/ 0 HIGH (9 MEDIUM/2 LOW pre-existing, không đổi) |
| Pending | **FID-VN-020** (CT-011, APPROVED — implement next session, ~180-220 LOC + tests) · **CT-054** 🟡 (regenerate `data/eval/bench_002b_results.json` baseline + debug Oresol NER FP `REF_HN_P1_Clip3.wav`) · **CT-053** Vietnamese Medical Phonetic Encoder (Phase 2, chờ pilot audio) · **CT-027** Ciprofloxacin "si pô lo siêu âm si" alias (cần thêm audio mẫu) · **CT-049** (Andy re-test clip TMH lần 3) · **PA-020/PA-021** (Andy test UI FID-VN-017/018) · **PA-015/PA-017/PA-018** (Andy test FID-VN-013/014/015/016 UI) · **CT-019** (🔴 A2 VAD-chunk regression) · CT-016/CT-017/CT-014/CT-035/CT-036/CT-037/CT-039/CT-044 ⏳ |
| Next task | **FID-VN-020** implement (ORCH-001/CT-011, đã approve) · **TRAIN-001** full fine-tune (9207 samples VietMed, nhiều epoch) khi có GPU/quota tốt hơn (Kaggle 30h/tuần) hoặc pilot audio · CT-054 regenerate baseline · CT-049 re-test · CT-019 debug A2-VAD nếu có audio · các PA UI test khác |
| Design | `docs/records/DESIGN_REPORT_v1.1_20260606.md` (§15 v2.1) · FID: `fids/FID-VN-007.md`, `fids/FID-VN-010.md`, `fids/FID-VN-012.md`, `fids/FID-VN-013.md`, `fids/FID-VN-014.md`, `fids/FID-VN-015.md`, `fids/FID-VN-016.md`, `fids/FID-VN-017.md`, `fids/FID-VN-018.md`, `fids/FID-VN-019.md`, `fids/FID-VN-020.md` |
