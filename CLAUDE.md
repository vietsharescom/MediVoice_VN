...
|---|---|
| Version | v0.11.28 |
| Status | **TRAIN-001 first Kaggle GPU run ✅ 2026-06-13** — 1 epoch fine-tune `vinai/PhoWhisper-medium` trên `combined_manifest.jsonl`, checkpoint-1151 downloaded → `models/asr_phowhisper/` (local only, gitignored). Eval n=30 (`vietmed_manifest.jsonl` slice): **WER 0.2968 → 0.1517** (`data/eval/train001_eval_baseline.json`/`train001_eval_results.json`). ⚠️ CHƯA đạt GO criteria đầy đủ (FID-VN-007 §Side effects — cần full-dataset eval + `ref_voice_manifest.jsonl` (57 clip BS thật) + Drug CEER <0.10) → `src/core/l1a_asr.py` MODEL_ID KHÔNG đổi, cần FID riêng sau khi đạt GO. CT-062 thêm vào BACKLOG cho phần eval còn lại. Cũng hỗ trợ BS pilot cài đặt: máy chưa có Python → `torch==2.3.0` không có wheel cho Python 3.13 → hướng dẫn cài Python 3.11 song song + `py -3.11 -m venv venv`. Phiên trước (v0.11.27): **CT-057** ✅ `save_recording()` lưu mọi lần `/api/transcribe` vào `data/recordings/`. |
| Tests | **1001/1001 PASS** (không đổi — session này chỉ docs + data/eval/ artifacts, không sửa code) · bandit src/ 0 HIGH (9 MEDIUM/2 LOW pre-existing, không đổi) |
| Pending | **CT-062** full eval model fine-tune (full vietmed_manifest 9207 samples + ref_voice_manifest 57 clip + Drug CEER) trước khi viết FID swap MODEL_ID · **CT-053** Vietnamese Medical Phonetic Encoder (Phase 2, chờ pilot audio) · **CT-027** Ciprofloxacin "si pô lo siêu âm si" alias (cần thêm audio mẫu) · **CT-049** (Andy re-test clip TMH lần 3) · **PA-020/PA-021** (Andy test UI FID-VN-017/018) · **PA-015/PA-017/PA-018** (Andy test FID-VN-013/014/015/016 UI) · **CT-019** (🔴 A2 VAD-chunk regression) · CT-016/CT-017/CT-014/CT-035/CT-036/CT-037/CT-039/CT-044 ⏳ · BS pilot đang cài Python 3.11 + venv trên máy (`install.bat` step [3/6] torch==2.3.0) |
| Next task | **CT-062** chạy full eval (vietmed_manifest đầy đủ + ref_voice_manifest + Drug CEER) cho checkpoint-1151 → nếu đạt GO (WER<0.20 + Drug CEER<0.10) viết FID swap `l1a_asr.py` MODEL_ID sang `models/asr_phowhisper/` · hỗ trợ BS pilot hoàn tất `install.bat` (Python 3.11 venv) · CT-049 re-test · CT-019 debug A2-VAD nếu có audio |
