...
|---|---|
| Version | v0.11.27 |
| Status | **CT-057 ✅ DONE 2026-06-13** — `src/core/l0_normalize.py::save_recording()` lưu MỌI lần `/api/transcribe` (audio + transcript_raw/corrected + form_data + confidence + route + dvp_specialty/region + dialect_subs) vào `data/recordings/{ts}_{record_id}.wav`+`.json` trước `purge_audio()` (Privacy by Design L0 không đổi, chỉ thêm 1 bản copy cho Andy đánh giá), `data/recordings/` thêm vào `.gitignore`. Cũng fix `notebooks/TRAIN_001_colab_kaggle.ipynb`: cell 8 `--batch 2 --grad-accum 4 --epochs 1` (per-device batch 8 OOM trên GPU 14.5GB T4/P100), cell 9 fallback sang `vietmed_manifest.jsonl` khi `ref_voice_manifest.jsonl` rỗng (audio gitignored); `scripts/train_asr_phowhisper.py` thêm `--grad-accum` + `gradient_checkpointing` khi `--fp16`. Phiên trước (v0.11.26): **CT-061** ✅ pilot audio retention opt-in (`retain_pilot_audio`, flag `pilot_audio_retention` trong `config/facility_config.json`, default `false`). |
| Tests | **1001/1001 PASS** (998 + 3 mới cho CT-057) · bandit src/ 0 HIGH (9 MEDIUM/2 LOW pre-existing, không đổi) |
