...
|---|---|
| Version | v0.11.18 |
| Status | **CT-051 DONE ✅ (2026-06-12)** — `_match_window()` (`src/core/l1b_drug_correct.py`) chỉ thử window 1-4 từ, nhưng `drug_db_v200.json` có **187 `phonetic_variants` dài 5-6 từ** (Azithromycin "a zi thro my xin", Ciprofloxacin "xi pro phlo xa xin", Atorvastatin, Rosuvastatin, Clarithromycin...) curated từ DRUG-ALIAS-001/CONS-20260610-001/002 nhưng KHÔNG THỂ MATCH. Mở rộng window → (6,5,4,3,2,1), exact-match nên không tăng false-positive risk. +2 tests. Phần CT-027(5) — Ciprofloxacin "si pô lo siêu âm si" (CT-016) vẫn chưa fix (spelling khác, cần thêm data). Trước đó cùng phiên: **TRAIN-001 Colab session** — đã chạy thử smoke-test trên Colab (85 samples VietMed `cv` split, `--batch 2`, 1 epoch) → pipeline OK (loss giảm 2.613), nhưng KHÔNG download checkpoint (chỉ là proof-of-pipeline trên 85 samples/1 epoch, gần như = base model, không có giá trị fine-tune thật — khuyên Andy hủy download 3GB/2h). Full fine-tune thật (9207 samples, nhiều epoch) để dành phiên sau khi có Kaggle/GPU mạnh hơn hoặc pilot audio. |
| Tests | **958/958 PASS** · bandit src/ 0 HIGH (9 MEDIUM/2 LOW pre-existing, không đổi) |
| Pending | **CT-042** 🔴 (BACKLOG IMMEDIATE — L1b phonological correction p↔b/t↔d/k↔g, cần FID trước khi sửa pipeline FROZEN) · **CT-027** Ciprofloxacin "si pô lo siêu âm si" alias (cần thêm audio mẫu) · **CT-049** (Andy re-test clip TMH lần 3) · **PA-020/PA-021** (Andy test UI FID-VN-017/018) · **PA-015/PA-017/PA-018** (Andy test FID-VN-013/014/015/016 UI) · **CT-019** (🔴 A2 VAD-chunk regression) · CT-016/CT-017/CT-014/CT-035/CT-036/CT-037/CT-039/CT-044 ⏳ |
| Next task | **CT-042** viết FID trước (🔴 HIGH, IMMEDIATE) · **TRAIN-001** full fine-tune (9207 samples VietMed, nhiều epoch) khi có GPU/quota tốt hơn (Kaggle 30h/tuần) hoặc pilot audio · CT-049 re-test · CT-019 debug A2-VAD nếu có audio · các PA UI test khác |
