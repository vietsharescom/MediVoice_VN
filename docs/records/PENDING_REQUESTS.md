...
| CT-019 | **🔴 A2 VAD-chunk regression** — Wire `vad_chunk_audio()`+`transcribe_chunks()` vào `/transcribe` → REVERTED ngay vì transcript "KHÔNG NHẬN DẠNG ĐƯỢC GÌ LUÔN" (test thực tế Andy). Hypothesis: PhoWhisper hallucinate nặng trên chunk ngắn độc lập + `initial_prompt` lặp lại mỗi chunk → bias sai. Cần debug offline (so sánh per-chunk transcript vs whole-file) trước khi wire lại — KHÔNG tự ý wire lại nếu chưa A/B test rõ ràng. | 🔴 HIGH | ⏳ PENDING | 2026-06-09 |
