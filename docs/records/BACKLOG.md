...
### CANDIDATE FID-VN-017/018 — Phonetic guidance + L1b phonological correction + DVP flow reorder [2026-06-11, chờ Claude draft FID]
> Andy feedback session 2026-06-11 (sau FID-VN-016 test) — 4 ý lớn, log ngay tránh mất:

- [ ] **CT-040** 🟡 Mở rộng `pronunciation_en` (USAN-style) ra ngoài 25 thuốc pilot noi_khoa — Andy nhấn mạnh đây là "mẫu định vị" quan trọng cho user, ưu tiên các specialty pilot khác (tim_mach, ...) trước khi làm hết 110-155 thuốc. (Phase 2 của CT-039)
- [ ] **CT-041** 🟡 Thiết kế lại `transliterate_to_vn_phonetic()` — hiện tại là heuristic respelling tự do, Andy muốn dòng phiên âm VN **hướng user đọc đúng theo tiếng Anh chuẩn** (không phải đọc tự do kiểu Việt hoá hoàn toàn), viết bằng chữ Việt dễ đọc nhưng định hướng âm gốc EN. User vẫn có thể tự đọc theo cách của mình (giữ cơ chế confirm cá nhân hoá FID-VN-016), nhưng default nên khuyến khích chuẩn. **Research cần làm**: tìm hệ quy ước phiên âm Anh→Việt chuẩn (vd sách dạy phát âm tiếng Anh cho người Việt) — CHƯA xác nhận có "từ điển Anh-Việt chuẩn phát âm thuốc y khoa" chính thức nào (xem CT-039 — kết luận trước đó là KHÔNG có).
...
- [ ] **CT-043** 🟡 **DVP setup flow reorder**: hiện tại Calibration Lab (vùng miền/giọng) độc lập với chọn specialty. Andy muốn: (1) BS chọn **chuyên khoa TRƯỚC** → narrow down phần setup giọng theo chuyên khoa (vd wordlist thuốc theo specialty, đã có ở Wizard nhưng cần áp dụng sớm hơn trong flow). (2) **Double-check vùng miền**: nếu DoctorProfile.region="central" (Trung) thì bài test Lab Phần 1/2 PHẢI viết theo phong cách/từ ngữ giọng Trung (hiện `lab-region-sentence`/`lab-passage-text` có vẻ cố định 1 bộ — cần kiểm tra `READING_PASSAGE_VI` trong `src/core/calibration_metrics.py` có đổi theo region không) để vừa test vừa xác nhận lại profile.region khớp giọng đọc thật. CHẠM DVP/Calibration Lab UI flow → cần FID (tương tự FID-VN-013/014/015/016).

**Next step**: Claude research (b/p, m/n, final-l trong ngữ âm Việt vs Anh — đã là kiến thức ngôn ngữ học phổ biến, không cần "từ điển" mới) + draft FID-VN-017 (CT-041+CT-042, phonetic dictionary + L1b correction) và FID-VN-018 (CT-040+CT-043, data expansion + DVP flow) cho Andy review trước khi implement.
