...
- [ ] **CT-040** 🟡 Mở rộng `pronunciation_en` (Merriam-Webster-style, xem CONS-20260611-001) ra ngoài 25 thuốc pilot noi_khoa — Andy nhấn mạnh đây là "mẫu định vị" quan trọng cho user, ưu tiên các specialty pilot khác (tim_mach, ...) trước khi làm hết 110-155 thuốc. (Phase 2 của CT-039) → đang làm trong `fids/FID-VN-017.md`
- [ ] **CT-041** 🟡 Thêm "stress hint" (viết hoa âm tiết trọng âm trong `vn_phonetic_default`, lấy từ `pronunciation_en`) + tip ngắn về phụ âm bật hơi p/t/k — hướng BS đọc gần đúng tiếng Anh chuẩn hơn mà vẫn dễ đọc bằng chữ Việt. User vẫn tự đọc theo cách của mình (confirm cá nhân hoá FID-VN-016), default chỉ là gợi ý. → đang làm trong `fids/FID-VN-017.md`
