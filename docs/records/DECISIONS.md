...
| 2026-06-11 | **PILOT PHASE EXCEPTION #2 (TRAIN-001)**: cho phép upload audio pilot có PII thật (giọng BS+BN) lên Google Colab/Kaggle (Google Cloud, ngoài VN) **TẠM THỜI** để chạy fine-tune PhoWhisper trên GPU free-tier | Andy xác nhận (2026-06-11): "tôi đã có consent nên không lo. làm đi" — cùng phạm vi consent đã ghi nhận ở exception 2026-06-10 (CT-024). KHÔNG đổi Rule #4 cho production. | **TẠM THỜI — chỉ training run, KHÔNG lưu lâu dài trên Colab/Kaggle.** Sau khi train xong: (1) xóa audio khỏi Colab/Kaggle runtime + Drive/Dataset upload ngay; (2) chỉ download checkpoint model (.bin/.safetensors — không chứa audio/PII) về máy local/VN cloud. VietMed (16h, MIT, public, KHÔNG PII) KHÔNG bị ảnh hưởng bởi exception này — luôn được phép trên Colab/Kaggle. Trước launch chính thức: production fine-tune (nếu cần lặp lại) PHẢI chạy trên VN Cloud GPU (VNG/FPT/VNPT), không Colab/Kaggle. |
